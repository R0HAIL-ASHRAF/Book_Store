...
template <class type>
void MyVector<type>::reserve(int new_capacity) {
    if (new_capacity == cap) return;
    if (new_capacity > cap) {
        type* newArr = new type[new_capacity];
        for (int i = 0; i < len; i++) {
            newArr[i] = arr[i];
        }
        delete[] arr;
        ...
        throw out_of_range("LESS CAPACITY THAN THE ACTUAL ONE");
    }
}
